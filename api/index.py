...
app = FastAPI(title="FIFA 2026 API")
...
def predict_group(grp: str):
    ...
    if not gt:
        return JSONResponse({"error": "Not found"}, status_code=404)
    return {"group": g, "predictions": pg(g, gt)}
...
def start_live(mid: int):
    ...
    GOAL = ["Goal!", "What a strike!", "Finds the net!", "Powerful shot!", "Clinical finish!"]
...
# Serve frontend static files in production (Vercel vercel-build)
